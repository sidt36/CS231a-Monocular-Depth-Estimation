...
from vision.generate_uncertainity_map import UncertaintyAutoencoder
import random
from vision.depth_anything import DepthEstimator
...
def main():
    ...
    if args.task == 'train' or args.task == 'test':
        ...
        uncertainity_model = UncertaintyAutoencoder(device='cpu')
        ...
        for set_num in range(n_sets):
            ...
            for attempt_id in range(attempts_per_set):
                ...
                uncertainity_prediction = uncertainity_model.predict(rgb_obs, depth_obs)
                
                uncertainty_binary = np.array(uncertainity_prediction < 90).astype(int)
                ...
                plt.subplot(2, 3, 2)
                plt.imshow(depth_obs, cmap='viridis')
                plt.title('Depth')
                plt.axis('off')
                
                plt.subplot(2, 3, 3)
                plt.imshow(uncertainity_prediction[0,0], cmap='hot')
                plt.title('Uncertainty Prediction')
                plt.axis('off')
                
                plt.subplot(2, 3, 4)
                plt.imshow(uncertainty_binary[0,0], cmap='gray')
                ...
                if not mask or not hasattr(mask, 'masks') or len(mask.masks) == 0:
                    ...
                    best_mask_idx = 0
                    best_uncertainty_percentage = 0
                    
                    for i, candidate_mask in enumerate(mask.masks):
                        binary_candidate = candidate_mask > 0
                        overlap = binary_candidate & uncertainty_binary
                        uncertainty_percentage = np.sum(overlap) / np.sum(binary_candidate) if np.sum(binary_candidate) > 0 else 0
                        
                        if uncertainty_percentage > best_uncertainty_percentage:
                            best_uncertainty_percentage = uncertainty_percentage
                            best_mask_idx = i
                    
                    print(f"Selected mask {best_mask_idx} with {best_uncertainty_percentage:.2%} uncertainty overlap")
